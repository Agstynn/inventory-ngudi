...
$result = mysqli_query($conn, "SELECT p.*, a.kode AS kode_alat FROM pemeliharaan_alat p LEFT JOIN alat a ON p.alat_id=a.id $w ORDER BY p.tanggal DESC");
?>
<div class="d-flex justify-content-between mb-3">
  <h2>Pemeliharaan Alat</h2>
  <div>
    <a href="form.php" class="btn btn-success">+ Tambah Pemeliharaan</a>
...
<table class="table table-bordered">
  <thead class="table-light"><tr><th>Tanggal</th><th>Nama Alat</th><th>No Seri</th><th>Ruangan</th><th>Jenis</th><th>Keterangan</th><th>Aksi</th></tr></thead>
  <tbody>
    ...
    <tr>
      ...
      <td><?=htmlspecialchars($row['jenis_pemeliharaan'])?></td>
      <td><?=htmlspecialchars($row['keterangan'])?></td>
      <td>
        <a href="form.php?id=<?=$row['id_pemeliharaan']?>" class="btn btn-sm btn-warning">Edit</a>
        <a href="delete.php?id=<?=$row['id_pemeliharaan']?>" onclick="return confirm('Hapus data?')" class="btn btn-sm btn-danger">Hapus</a>
      </td>
    </tr>
    <?php endwhile; ?>
  </tbody>
